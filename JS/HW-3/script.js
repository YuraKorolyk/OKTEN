...
for (let i = 0; i < 10; i++) {
   document.write (`<div>hello, ${i+1}</div>`);
}
...
let m = 0;
while(m < 20) {
   document.write(`<h1>world, ${m+1}</h1>`);
   m++;
}
...
for (let i = 0; i < users.length; i++) {
   if (users[i].status === true) {
      console.log(`Status true: ${users[i].name}`);
   } else if (users[i].status === false) {
      console.log(`Status false: ${users[i].name}`);
   }
   if (users[i].age > 30) {
      console.log(`Age > 30: ${users[i].name}`);
   }
}